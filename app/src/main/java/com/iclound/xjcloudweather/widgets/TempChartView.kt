...
class TempChartView @JvmOverloads constructor(
    ...
    init {
        topBottom = DisplayUtils.dp2px(8f)
        mTextPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        mTextPaint.textSize = DisplayUtils.sp2px(App.context, 13f).toFloat()
        mTextPaint.typeface = Typeface.DEFAULT_BOLD
        mTextPaint.color = resources.getColor(R.color.weather_temp, null)
        textHeight = (mTextPaint.fontMetrics.bottom - mTextPaint.fontMetrics.top).toInt()

        val lineWidth = DisplayUtils.dp2px(2f)

        mLowPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        mLowPaint.style = Paint.Style.STROKE
        mLowPaint.isFakeBoldText = true
        mLowPaint.strokeWidth = lineWidth.toFloat()
        // 设置线帽，方式折线陡峭时线中间出现裂痕
        mLowPaint.strokeCap = Paint.Cap.ROUND
        mLowPaint.color = resources.getColor(R.color.weather_line, null) //Color.parseColor("#383838")

        mHighPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        mHighPaint.strokeWidth = lineWidth.toFloat()
        mHighPaint.strokeCap = Paint.Cap.ROUND
        mHighPaint.style = Paint.Style.STROKE
        mHighPaint.isFakeBoldText = true
        mHighPaint.color = resources.getColor(R.color.weather_line, null) //Color.parseColor("#383838")

        mCirclePaint = Paint(Paint.ANTI_ALIAS_FLAG)
        mCirclePaint.strokeWidth = lineWidth.toFloat()
        mCirclePaint.strokeCap = Paint.Cap.ROUND
        mCirclePaint.style = Paint.Style.STROKE
        mCirclePaint.color = resources.getColor(R.color.weather_circle, null) //Color.parseColor("#383838")

        pntRadius = DisplayUtils.dp2px(3f).toFloat()
    }
...
//        LogUtil.e("min: " + minTemp + " max:" + maxTemp + " prev:" + prev + " curr: " + current + " next: " + next);
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawCircle(0f, topY.toFloat(), pntRadius, mCirclePaint )
        canvas.drawCircle(0f, bottomY.toFloat(), pntRadius, mCirclePaint )
        canvas.drawText( highText, ( -lowTextWidth / 2).toFloat(), topY - mTextPaint.fontMetrics.bottom * 2, mTextPaint )
        canvas.drawText( lowText, ( -lowTextWidth / 2).toFloat(), (bottomY + textHeight).toFloat(), mTextPaint )
        // 绘制当前点给前一天数据的连线
        if (mPrev != null) {
            val prev = getEnds(mPrev!!)
            //Log.d("", "Ends: - prev: startY:" + (prev.first + topY ) / 2f + " topY:" + topY + " density:"+density )
            canvas.drawLine( -mHalfWidth , (prev.first + topY ) / 2f, 0f - pntRadius*1.5f, topY.toFloat(), mHighPaint )
            canvas.drawLine( -mHalfWidth , (prev.second + bottomY) / 2f, 0f - pntRadius*1.5f, bottomY.toFloat(), mLowPaint )
        }
        // 绘制当前点给后一天数据的连线
        if (mNext != null) {
            val next = getEnds(mNext!!)
            ///Log.d("", "Ends: - next: startY:" + topY + " stopY:" + (next.first + topY) / 2f+ " density:"+density)
            canvas.drawLine(0f + pntRadius*1.5f, topY.toFloat(), mHalfWidth, (next.first + topY) / 2f, mHighPaint )
            canvas.drawLine(0f + pntRadius*1.5f , bottomY.toFloat(), mHalfWidth, (next.second + bottomY) / 2f, mLowPaint )
        }
    }
    ...
    private fun getEnds(daily: Daily): Pair<Int, Int> {
        ...
        //Log.d("", "Ends:" + topY + " maxTemp:" + maxTemp +" max:" + daily.tempMax + " min:" + daily.tempMin)
